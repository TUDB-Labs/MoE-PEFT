...
def test_gemma_eager_minimal_forward(dtype):
    ...
    hf_model = hf_gemma.GemmaForCausalLM(cfg).to(dtype)
    moe = MoeGemma.from_pretrained(
...
# Note: Phi eager path currently requires modeling fix for eager_attention signature.
# Phi coverage is provided in flash-attn tests; skipping duplicate eager test here.


@pytest.mark.parametrize("dtype", [torch.float32])
def test_phi_eager_minimal_forward(dtype):
